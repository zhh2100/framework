...
use pidan\route\RuleGroup;
use pidan\route\RuleItem;
use pidan\route\RuleName;
use pidan\route\Url as UrlBuild;
use pidan\route\ResourceRegister as Resource;
 ...
 * @method static \pidan\Route lazy(bool $lazy = true) 设置路由域名及分组（包括资源路由）是否延迟解析
 * @method static void setTestMode(bool $test) 设置路由为测试模式
 * @method static bool isTest() 检查路由是否为测试模式
 * @method static \pidan\Route mergeRuleRegex(bool $merge = true) 设置路由域名及分组（包括资源路由）是否合并解析
 ...
 * @method static \pidan\Route pattern(array $pattern) 注册变量规则
 * @method static \pidan\Route option(array $option) 注册路由参数
 ...
 * @method static \pidan\Route bind(string $bind, string $domain = null) 设置路由绑定
 ...
 * @method static \pidan\Route setCrossDomainRule(Rule $rule) 设置跨域有效路由规则
 ...
 * @method static \pidan\Route rest(string|array $name, array|bool $resource = []) rest方法定义和修改
 ...
 * @method static Response dispatch(\pidan\Request $request, \Closure|bool $withRoute = true) 路由调度
