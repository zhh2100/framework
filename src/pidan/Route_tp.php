...
class Route
{
	...
	{
		// 自动检测域名路由  转换PATH_INFO分隔符,拼接url
		$url = str_replace($this->config['pathinfo_depr'], '|', $this->path());//多应用 index|index
	...
	protected function path(): string
	{
		...
		if (false === $suffix) {
			...
			// 去除正常的URL后缀
			$path = preg_replace('/\.(' . $suffix . ')$/i', '', $pathinfo);
		} else {
			// 允许任何后缀访问
